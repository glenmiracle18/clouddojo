...
export default function FilterComponent({ onFilter }: FilterComponentProps) {
  ...
  const activeFilterCount = selectedTopics.length + (selectedLevel !== "all" ? 1 : 0)
        ...
        <PopoverContent className="w-[320px] p-4" align="start">
          <div className="space-y-4">
            ...
            <div>
              ...
              <RadioGroup value={selectedLevel} onValueChange={handleLevelChange} className="flex flex-col space-y-1">
        ...
        <div className="flex flex-wrap gap-1.5">
          {selectedTopics.map((topicId) => {
            ...
            return (
              <Badge key={topicId} variant="secondary" className="flex items-center gap-1">
                {topic?.label}
                <button onClick={() => handleTopicChange(topicId)} className="ml-1 rounded-full hover:bg-muted p-0.5">
          ...
          {selectedLevel !== "all" && (
            <Badge variant="secondary" className="flex items-center gap-1">
              ...
              <button onClick={() => setSelectedLevel("all")} className="ml-1 rounded-full hover:bg-muted p-0.5">
                <Check className="h-3 w-3" />
              </button>
            </Badge>
          )}
