...
export function ResizableNavbar({children, className}: {children: React.ReactNode, className?: string}) {
  ...
  return (
    <motion.div
      ...
      className={cn("inset-x-0 top-10 z-40 w-full fixed bg-transparent", className)}
    >
      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full text-gray-900 rounded-full bg-gray-100 dark:bg-neutral-800"
            />
          )}
